...
func getWeaponPerks(ctx context.Context, database *db.DB, logger *utils.Logger, redis *redis.Client, perkHashIDs []string) (*filteredPerksResult, error) {
	...
	if err == nil {
		...
		if err := json.Unmarshal([]byte(cached), &wrap); err == nil {
			...
			if age <= freshFor {
				logger.PrintColour(true, "brightBlack", "CACHE HIT fresh getWeaponPerks %s", cacheKey)
				shouldLog = false
				v := wrap.Value
				return &v, nil
			}
			if age <= staleFor {
				logger.PrintColour(true, "brightBlack", "CACHE HIT stale getWeaponPerks %s", cacheKey)
				shouldLog = false
