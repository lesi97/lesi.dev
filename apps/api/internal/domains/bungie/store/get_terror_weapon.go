...
import (
	...
	bungie_utils "github.com/lesi97/lesi.dev/internal/domains/bungie/utils"
)

func (s *Store) GetTerrorWeapon(ctx context.Context) (*string, error) {
	request, ok := ctx.Value(bungieContextKey).(BungieContextInfo)
	if !ok {
		return nil, fmt.Errorf("invalid context")
	}

	weaponData, err := bungie_utils.GetTerrorWeaponData(request.WeaponName)
	if err != nil {
		return nil, err
	}
