...
func (h *Handler) HandleUpdatePlexLabels(w http.ResponseWriter, r *http.Request) {

	status, _, err := h.httpUtils.ValidateRequest(r)
	if err != nil {
	...
	if strings.ToLower(plexData.Event) != "library.new" {
		utils.Success(w, http.StatusNoContent, nil)
		return
	}

	err = h.store.UpdatePlexLabels(ctx, plexData)
	if err != nil {
		message := fmt.Sprintf("%v", err)
		h.logger.SendDiscordNotification(utils.SendDiscordNotificationArgs{
			Content:  message,
			Username: discord_username,
			Title:    "Failed to update Plex Labels",
		})
		h.logger.Error(message)
		utils.Error(w, http.StatusInternalServerError, "failed to update plex labels")
		return
	}
