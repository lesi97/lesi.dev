...
import { WeaponSlotEnum } from '@/lib/server/data/bungie/bungie.enum';
import { NextApiRequest, NextApiResponse } from 'next';

export default async function GET(req: NextApiRequest, res: NextApiResponse) {
    const { bungie_id, platform } = req.query;
    const platformEnum = getPlatformEnum(platform);
    const user = await Bungie_Current_User_Data.create(bungie_id, platformEnum, WeaponSlotEnum.Kinetic);
    const data = user.current_weapon;
